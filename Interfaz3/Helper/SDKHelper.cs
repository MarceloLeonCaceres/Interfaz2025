...
namespace SDK
{
    public class SDKHelper : ISDKHelper
    {
        ...
        private static Utilitarios.ClsEncriptacion Encripter = new Utilitarios.ClsEncriptacion();
        ...
        public string bio_GetNumeroDeSerie(out string sSN)
        {
            axCZKEM1.GetSerialNumber(GetMachineNumber(), out sSN);
            return sSN;
        }

        public int sta_GetDeviceInfo(out int idMaquina, out string sFirmver, out string sMac, out string sPlatform, out string sSN, out string sProductTime, out string sDeviceName, out int iFPAlg, out int iFaceAlg, out string sProducter)
        {
            int iRet = 0;
            ...
            */
            ...
            axCZKEM1.GetDeviceInfo(GetMachineNumber(), 2, ref idMaquina);
        ...
        // Por hacer
        // Modificación
        //public int bio_BorrarEnrollData(List<string> Badgenumbers)
        //{
        //    if(GetConnectState() == true)
        //    {
        ...
        public int sta_DeleteEnrollData(DataGridView dgvBitacora, ProgressBar prgSta, int iOpcion = 0, List<string> lstEmpleados = null)
        {
            if (GetConnectState() == false)
            {
                if (iOpcion == 0)
                    reporta(dgvBitacora, "No hay equipos conectados");
                return -1024;
            }

            int idwErrorCode = 0;
            int iBackupNumber = 12;
            foreach (string sUserID in lstEmpleados)
            {
                if (axCZKEM1.SSR_DeleteEnrollData(iMachineNumber, sUserID.Trim(), iBackupNumber))
                {
                    axCZKEM1.RefreshData(iMachineNumber);//the data in the device should be refreshed
                    reporta(dgvBitacora, "Eliminado el usuario: " + sUserID);
                }
                else
                {
                    axCZKEM1.GetLastError(ref idwErrorCode);
                    reporta(dgvBitacora, "Error " + idwErrorCode.ToString() + " al eliminar el usuario: " + sUserID);
                }
            }
        ...
        private void reporta(DataGridView dgvBitacora, string evento)
        {
            dgvBitacora.Rows.Add(dgvBitacora.Rows.Count + 1, evento, "", System.DateTime.Now.ToString());
            dgvBitacora.Refresh();
        }

        #region UserFP
        public int sta_GetAllUserFPInfo(DataGridView dgvBitacora, ProgressBar prgSta, DataGridView dgvUsuariosLeidos,
            DataTable dtEmpleados, int iOpcion = 0, List<string> codigosEmpleados = null)
        {
            if (GetConnectState() == false)
            {
                if (iOpcion == 0)
            ...
            int num = 0;
            int iFpCount = 0;
            int index = 0;  // Se usaba para pasar a la siguiente fila del listView
            int xx = 1;
            #endregion

            dgvUsuariosLeidos.Rows.Clear();
            dgvUsuariosLeidos.Refresh();
            ...
            if (codigosEmpleados == null)
            {
                while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
                {
                    ...
                    for (idwFingerIndex = 0; idwFingerIndex < 10; idwFingerIndex++)
                    {
                        ...
                        if (axCZKEM1.GetUserTmpExStr(iMachineNumber, sEnrollNumber, idwFingerIndex, out iFlag, out sFPTmpData, out iFPTmpLength))
                        {
                            dgvUsuariosLeidos.Rows.Add(sEnrollNumber, bEnabled.ToString(), sName, sCardnumber, sPassword, idwFingerIndex.ToString(), iFlag.ToString(), sFPTmpData, iPrivilege.ToString());

                            //if (xx == 1)
                            //{
                            //    dgvLeeUsuarios.Rows.Add(sEnrollNumber, bEnabled.ToString(), sName, sCardnumber, sPassword, idwFingerIndex.ToString(), iFlag.ToString(), sFPTmpData, iPrivilege.ToString());
                            //}
                            //else
                            //{
                            //    dgvLeeUsuarios.Rows.Add(sEnrollNumber, bEnabled.ToString(), sName, sCardnumber, sPassword, idwFingerIndex.ToString(), iFlag.ToString(), sFPTmpData, iPrivilege.ToString());
            ...
            else
            {
                while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
                {
                    ...
                    if (codigosEmpleados.Contains(sEnrollNumber))
                    {
                        ...
                        //emp.pin = sEnrollNumber;
                        //emp.name = sName;
                        //emp.privilege = iPrivilege;
                        //emp.password = sPassword;
                        //emp.cardNumber = sCardnumber;
                        //datosEmpleados.Add(emp);

                        DataRow dr = dtEmpleados.NewRow();
                        dr["pin"] = sEnrollNumber;
                        dr["name"] = sName;
                        dr["password"] = sPassword;
                        dr["privilege"] = iPrivilege;
                        dr["cardNumber"] = sCardnumber;
                        dr["enprivilege"] = bEnabled;
                        dtEmpleados.Rows.Add(dr);
            ...
            //prgSta.Value = 100;
            if (iOpcion == 0)
                reporta(dgvBitacora, "Download user count : " + num.ToString() + " ,  fingerprint count : " + iFpCount.ToString());
        ...
        public int ConsultaUsuariosReloj(DataTable dtUsersReloj, out int numUsuarios)
        {
            numUsuarios = 0;    // Es el número de empleados en el biométrico
            if (GetConnectState() == false)
            {
                // reporta(dgvBitacora, "No hay equipos conectados");
                return -1024;
            ...
            while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
            {
                dtUsersReloj.Rows.Add(sEnrollNumber, sName, iPrivilege);
            }
        ...
        public int bio_TraeFPSeleccionados(List<string> lstEmpleados, DataGridView dgvUsuariosLeidos, out int numHuellas, out int numUsuarios)
        {
            numHuellas = 0;
            numUsuarios = 0;    // Es el número de empleados en el biométrico
            if (GetConnectState() == false)
            ...
            dtHuellas.Columns.Add("Badgenumber", typeof(String));
            dtHuellas.Columns.Add("enabled", typeof(String));
            dtHuellas.Columns.Add("Nombre", typeof(String));
            dtHuellas.Columns.Add("Card", typeof(String));
            dtHuellas.Columns.Add("Password", typeof(String));
            dtHuellas.Columns.Add("idFinger", typeof(String));
            dtHuellas.Columns.Add("iFlag", typeof(String));
            dtHuellas.Columns.Add("Template", typeof(String));
            dtHuellas.Columns.Add("privilegio", typeof(String));
            ...
            while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
            {
                if (lstEmpleados.Contains(sEnrollNumber))
                {
                    ...
                    for (idwFingerIndex = 0; idwFingerIndex < 10; idwFingerIndex++)
                    {
                        ...
                        if (axCZKEM1.GetUserTmpExStr(iMachineNumber, sEnrollNumber, idwFingerIndex, out iFlag, out sFPTmpData, out iFPTmpLength))
                        {
                            dtHuellas.Rows.Add(sEnrollNumber, "True", sName, "0", sPassword, idwFingerIndex.ToString(), iFlag.ToString(), sFPTmpData, iPrivilege.ToString());
                            // dgvUsuariosLeidos.Rows.Add(sEnrollNumber, bEnabled.ToString(), sName, sCardnumber, sPassword, idwFingerIndex.ToString(), iFlag.ToString(), sFPTmpData, iPrivilege.ToString());
                            dgvUsuariosLeidos.DataSource = dtHuellas;
                            numHuellas++;
                        }
                    }
                }
            ...
            // reporta(dgvBitacora, "Download user count : " + numUsuarios.ToString() + " ,  fingerprint count : " + iFpCount.ToString());
        ...
        public int bio_GetAllUserInfo(DataGridView dgvUserInfo)
        {
            if (GetConnectState() == false)
            {
                return -1024;
            ...
            {
                axCZKEM1.GetStrCardNumber(out sCardnumber);//get the card number from the memory         
                dgvUserInfo.Rows.Add(sEnrollNumber, bEnabled, sName, sPassword, iPrivilege, sCardnumber);
            }
        ...
        public int sta_SetAllUserFPInfo(ProgressBar prgSta, List<DataGridViewRow> lvUserInfo)
        {
            if (GetConnectState() == false)
            {
                return -1024;
            ...
            axCZKEM1.EnableDevice(iMachineNumber, false);
            int i = 0;
            foreach (DataGridViewRow fila in lvUserInfo)
            {
                ConvierteFilaEnValores_UserFpInfo(out sEnrollNumber, out sEnabled, out bEnabled, out sName, out sCardnumber, out sPassword,
                    out sdwFingerIndex, out sFlag, out sFPTmpData, out iPrivilege, fila);
                ...
                {
                    axCZKEM1.GetLastError(ref idwErrorCode);
                    // la línea siguiente sí estaba
                    // lblOutputInfo.Items.Insert(0, "*Upload user " + sEnrollNumber + " error,ErrorCode=!" + idwErrorCode.ToString());
                    //axCZKEM1.EnableDevice(iMachineNumber, true);
                    //return -1022;
                }
                i++;
            ...
            axCZKEM1.EnableDevice(iMachineNumber, true);
            // la línea siguiente sí estaba
            // lblOutputInfo.Items.Insert(0, "Uploading user successfully");

            return 1;
        }

        private static void ConvierteFilaEnValores_UserFpInfo(out string sEnrollNumber, out string sEnabled, out bool bEnabled, out string sName, out string sCardnumber, out string sPassword, out string sdwFingerIndex, out string sFlag, out string sFPTmpData, out int iPrivilege, DataGridViewRow fila)
        {
            sEnrollNumber = fila.Cells[0].Value.ToString();
            sEnabled = fila.Cells[1].Value.ToString();
            bEnabled = sEnabled.ToLower() == "1";
            sName = fila.Cells[2].Value.ToString();
            sCardnumber = fila.Cells[3].Value.ToString();
            sPassword =  String.IsNullOrWhiteSpace(fila.Cells[4].Value.ToString()) ? "" : Encripter.Desencripta(fila.Cells[4].Value.ToString());
            sdwFingerIndex = fila.Cells[5].Value.ToString();
            sFlag = fila.Cells[6].Value.ToString();
            sFPTmpData = fila.Cells[7].Value.ToString();
            iPrivilege = Convert.ToInt32(fila.Cells[8].Value.ToString());
        }

        public int sta_batch_SetAllUserFPInfo(ProgressBar prgSta, List<DataGridViewRow> lvUserInfo)
        {
            if (GetConnectState() == false)
            {
                return -1024;
            ...
            if (axCZKEM1.BeginBatchUpdate(iMachineNumber, 1))//create memory space for batching data
            {
                foreach (DataGridViewRow fila in lvUserInfo)
                {
                    ConvierteFilaEnValores_batch_UserFpInfo(out sEnrollNumber, out sEnabled, out bEnabled, out sName, out sCardnumber, out sPassword, out sdwFingerIndex, out sFlag, out sFPTmpData, out iPrivilege, fila);
            ...
            axCZKEM1.RefreshData(iMachineNumber);//the data in the device should be refreshed
            axCZKEM1.EnableDevice(iMachineNumber, true);
            // la siguiente línea funcionaba bien
            //lblOutputInfo.Items.Insert(0, "Upload user successfully in batch");
            return 1;
        }

        private static void ConvierteFilaEnValores_batch_UserFpInfo(out string sEnrollNumber, out string sEnabled, out bool bEnabled, out string sName, out string sCardnumber, out string sPassword, out string sdwFingerIndex, out string sFlag, out string sFPTmpData, out int iPrivilege, DataGridViewRow fila)
        {
            sEnrollNumber = fila.Cells["Userid"].Value.ToString();
            sEnabled = fila.Cells[1].Value.ToString();
            bEnabled = sEnabled.ToLower() == "1";

            sName = fila.Cells[2].Value.ToString();
            sCardnumber = fila.Cells[3].Value.ToString();
            sPassword = Encripter.Desencripta(fila.Cells[4].Value.ToString());
            sdwFingerIndex = fila.Cells[5].Value.ToString();
            sFlag = fila.Cells[6].Value.ToString();
            sFPTmpData = fila.Cells[7].Value.ToString();
            iPrivilege = Convert.ToInt32(fila.Cells[8].Value.ToString());
        ...
        public int subeHuellasLote(DataTable dt)
        {
            if (GetConnectState() == false)
            {
                return -1024;
            ...
            if (axCZKEM1.BeginBatchUpdate(iMachineNumber, 1))//create memory space for batching data
            {
                ...
                {
                    sEnrollNumber = fila[0].ToString();
                    //sEnabled = fila[1].ToString();
                    ...
                    //    bEnabled = false;
                    //}
                    sName = fila[2].ToString();
                    sCardnumber = fila[3].ToString();
                    sPassword = fila[4].ToString();
                    sdwFingerIndex = fila[5].ToString();
                    sFlag = fila[6].ToString();
                    sFPTmpData = fila[7].ToString();
                    iPrivilege = Convert.ToInt32(fila[8]);

                    //if (sCardnumber != "" && sCardnumber != "0")
                    //{
                    //    axCZKEM1.SetStrCardNumber(sCardnumber);
                    //}
                    if (axCZKEM1.SSR_SetUserInfo(iMachineNumber, sEnrollNumber, sName, sPassword, iPrivilege, true))//upload user information to the device
                    {
                        if (sdwFingerIndex != "" && sFlag != "" && sFPTmpData != "")
                        {
                            idwFingerIndex = Convert.ToInt32(sdwFingerIndex);
        ...
        public int sta_GetAllUserFaceInfo(DataGridView dgvBitacora, ProgressBar prgSta, DataGridView dgvUsuariosLeidos, int iOpcion = 0, List<string> lstEmpleados = null)
        {
            ...
            if (lstEmpleados == null)
            {
                while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
                {
                    ...
                    {
                        dgvUsuariosLeidos.Rows.Add(sEnrollNumber, true, sName, sPassword, iPrivilege, iLength, sTmpData);
                        num++;
                        prgSta.Value = num % 100;
                    }
                    prgSta.Value = num % 100;
                ...
                while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
                {
                    if (lstEmpleados.Contains(sEnrollNumber))
                    {
                        if (axCZKEM1.GetUserFaceStr(iMachineNumber, sEnrollNumber, iFaceIndex, ref sTmpData, ref iLength))//get the face templates from the memory
                        {
                            dgvUsuariosLeidos.Rows.Add(sEnrollNumber, true, sName, sPassword, iPrivilege, iLength, sTmpData);
                            num++;
                            prgSta.Value = (int)((num * 100) / lstEmpleados.Count);
                        }
                    }
                }
            }
            prgSta.Value = 100;
            if (iOpcion == 0)
                reporta(dgvBitacora, "Download user count : " + num.ToString() + " ,  faces count : " + num.ToString());
        ...
        public int sta_SetAllUserFaceInfo(ProgressBar prgSta, List<DataGridViewRow> lvUserInfo)
        {
            if (GetConnectState() == false)
            {
                return -1024;
            ...
            foreach (DataGridViewRow fila in lvUserInfo)
            {
                sEnrollNumber = fila.Cells[1].Value.ToString();
                sEnabled = fila.Cells[2].Value.ToString();
                bEnabled = sEnabled.ToLower() == "1" ? true : false;

                sName = fila.Cells[3].Value.ToString();
                sPassword = fila.Cells[4].Value.ToString();
                iLength = Convert.ToInt32(fila.Cells[6].Value.ToString());
                sTmpData = fila.Cells[7].Value.ToString();
                iPrivilege = Convert.ToInt32(fila.Cells[8].Value.ToString());
        ...
        */
        ...
        //            }
        //        }
        //        if (aBioType == 1)   //Indica la adquisición de la plantilla de huellas dactilares
        //        {
        //            break;
        //        }
        //    }
        ...
        #endregion
        public int sta_GetAllUserTarjetaInfo(DataGridView dgvBitacora, ProgressBar prgSta, DataGridView dgvUsuariosLeidos,
            DataTable dtEmpleados, int iOpcion = 0, List<string> codigosEmpleados = null)
        {
            if (GetConnectState() == false)
            {
            ...
            # region InializaParametros
            string sEnrollNumber, sName, sPassword, sCardnumber;
            bool bEnabled = false;
            int iPrivilege = 0;
            #endregion
            ...
            while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))//get all the users' information from the memory
            {
                if (codigosEmpleados.Contains(sEnrollNumber))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sCardnumber) && sCardnumber != "0")
                    {
                        DataRow dr = dtEmpleados.NewRow();
                        dr["pin"] = sEnrollNumber;
                        dr["name"] = sName;
            ...
            //}
            if (iOpcion == 0)
                reporta(dgvBitacora, "Usuarios con tarjeta: " + dtEmpleados.Rows.Count.ToString());
        ...
        // Progress Bar
        public clsIntString bio_LeeMarcaciones(IProgress<int> barraProgreso, Label etiqueta, DataTable dt_log, DataGridView dgvUserinfo, CancellationToken cancelToken)
        {
            clsIntString retorno = new clsIntString();
            // parámetro SerialNumber
            retorno.sn = "";

            if (GetConnectState() == false)
            {
                retorno.respuesta = -1024;
                return retorno;
            }
            ...
            axCZKEM1.GetSerialNumber(GetMachineNumber(), out sSN);
            retorno.sn = sSN;
            barraProgreso.Report(5);

            axCZKEM1.ReadAllUserID(iMachineNumber);//read all the user information to the memory  except fingerprint Templates
            barraProgreso.Report(10);
        ...
        public async Task Run(ProgressBar progressBar, Label label, DataTable data, DataGridView dataGrid)
        {
            ...
            try
            {
                clsIntString ejemplo;
                await Task.Run(() => ejemplo = bio_LeeMarcaciones(progress, label, data, dataGrid, cancelToken));
            }
            catch (Exception)
            {
                throw;
        ...
        public clsIntString bio_LeeMarcaciones_ProgressBar(ProgressBar progressB, DataTable dt_log, DataGridView dgvUserinfo, IProgress<int> progress, CancellationToken cancelToken, int nMarcaciones)
        {
            clsIntString retorno = new clsIntString();
            // parámetro SerialNumber
            retorno.sn = "";
            ...
            // Avance Barra de Progreso: Lee el número de serie del reloj
            progress?.Report(25);
            ...
            if (axCZKEM1.ReadGeneralLogData(GetMachineNumber()))
            {
                            ...
                            out idwInOutMode, out idwYear, out idwMonth, out idwDay, out idwHour, out idwMinute, out idwSecond, ref idwWorkcode))//get records from the memory
                {
                    ...
                    if (dt_log.Rows.Count % paso == 0)
                        progress?.Report(dt_log.Rows.Count / paso);
                }
                ret = 1;
            }
            else
        ...
        public clsIntString bio_LeeMarcaciones(DataTable dt_log, DataGridView dgvUserinfo)
        {
            ...
            axCZKEM1.EnableDevice(GetMachineNumber(), false);//disable the device
            LogHelpers.ReportaNovedad("Desactiva reloj");

            #region Establece Parametros
            // parametros Marcaciones
            string sdwEnrollNumber = "";
            int idwVerifyMode = 0;
            int idwInOutMode = 0;
            ...
            //List<string> listaTarjetasValidas = await LogicaTarjetas.GetTarjetasValidas();
            ...
            while (axCZKEM1.SSR_GetAllUserInfo(iMachineNumber, out sEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))
            {
                ...
                //if (!string.IsNullOrWhiteSpace(sCardnumber) && sCardnumber != "0")
                //{
                //    TarjetaDe[sEnrollNumber] = sCardnumber;
                //}
                dgvUserinfo.Rows.Add(sEnrollNumber, bEnabled, sName, sPassword, iPrivilege, sCardnumber);
            }

            LogHelpers.ReportaNovedad($"AllUserInfo {dgvUserinfo.Rows.Count}");

            StringBuilder sbMarcacionesDefectuosas = new StringBuilder();
            uint number;
            DateTime marcacion;
            string sFecha, fila;
            //int contadorMarcacionesTarjetaIlegal = 0;
            if (axCZKEM1.ReadGeneralLogData(GetMachineNumber()))
            {
                LogHelpers.ReportaNovedad("Ingresó a Lectura General de LogData");
                while (axCZKEM1.SSR_GetGeneralLogData(GetMachineNumber(), out sdwEnrollNumber, out idwVerifyMode,
                        out idwInOutMode, out idwYear, out idwMonth, out idwDay, out idwHour, out idwMinute, out idwSecond, ref idwWorkcode))//get records from the memory
                {
                    sFecha = idwYear + "-" + idwMonth.ToString("00") + "-" + idwDay.ToString("00") + " " + idwHour.ToString("00") + ":" + idwMinute.ToString("00") + ":" + idwSecond.ToString("00");
                    if (uint.TryParse(sdwEnrollNumber, out number) && DateTime.TryParse(sFecha, out marcacion) && sdwEnrollNumber.Length <= 9)
                    {
                        //if (idwVerifyMode == 4 && listaTarjetasValidas.Contains(TarjetaDe[sdwEnrollNumber].Substring(0,5)) == false)
                        //{
                        //    contadorMarcacionesTarjetaIlegal++;
                        //    continue;
                        //}
                        DataRow dr = dt_log.NewRow();
                        dr["User ID"] = sdwEnrollNumber;
                        ...
                        //try
                        //{
                        //    marcacion = new DateTime(idwYear, idwMonth, idwDay, idwHour, idwMinute, idwSecond);
                        //}
                        //catch (Exception)
                        //{
                        //    string detalle = "EnrollNumber: " + sdwEnrollNumber +
                        //        ", year: " + idwYear + ", month: " + idwMonth + ", day: " + idwDay +
                        //        ", hour: " + idwHour + ", minute: " + idwMinute + ", second: " + idwSecond +
                        //        ", VerifyMode: " + idwVerifyMode + ", OutMode: " + idwInOutMode + ", Workcode: " + idwWorkcode;
                        //    throw new FechaNoValidaException(detalle);
                        //}                    

                        dr["Verify Date"] = sFecha;
                        dr["Verify Type"] = idwVerifyMode;
                        dr["Verify State"] = idwInOutMode;
                        dr["WorkCode"] = idwWorkcode;
                        dr["sn"] = sSN;
                    ...
                    {
                        fila = sdwEnrollNumber + " " + sFecha + " " + idwVerifyMode + " " + idwInOutMode + " " + idwWorkcode + "\n";
                        sbMarcacionesDefectuosas.Append(fila);
                    }
                }

                LogHelpers.ReportaNovedad($"Get General de LogData {dt_log.Rows.Count}");
        ...
        public int sta_GetDeviceTime(Label lbDeviceTime, out DateTime fechaHora)
        {
            fechaHora = new DateTime(1900, 1, 1, 0, 0, 1);
            if (GetConnectState() == false)
            {
                // lblOutputInfo.Items.Add("*Please connect first!");
                return -1024;
            ...
            {
                fechaHora = new DateTime(idwYear, idwMonth, idwDay, idwHour, idwMinute, idwSecond);
                lbDeviceTime.Text = fechaHora.ToString();
        ...
        public int sta_btnRestartDevice()
        {
            if (GetConnectState() == false)
            {
                // lblOutputInfo.Items.Insert(0, "*Please connect first!");
                return -1024;
            }
            ...
            {
                sta_DisConnect();
                // lblOutputInfo.Items.Insert(0, "The device will restart");
                return 1;
            }
            else
            {
                axCZKEM1.GetLastError(ref idwErrorCode);
                // lblOutputInfo.Items.Insert(0, "*Operation failed,ErrorCode=" + idwErrorCode.ToString());
                return idwErrorCode;
            }
        ...
        public int sta_btnPowerOffDevice()
        {
            ...
            {
                sta_DisConnect();
                // lblOutputInfo.Items.Insert(0, "Power off device");
                return 1;
            }
            else
            {
